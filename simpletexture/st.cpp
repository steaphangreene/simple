    ...
    if (!src) {
      ...
      if (!strncmp((char *)head, "BLP1", 4)) {
        ...
        if (!uncomp) {
          ...
          SDL_RWseek(file, 0x9C, SEEK_SET);
          src = SDL_CreateRGBSurface(SDL_SWSURFACE, xs, ys, 32,
          ...
          for (Uint32 pix = 0; pix < xs * ys; ++pix) {
            pt.x = pix % xs;
            pt.y = pix / xs;
            SDL_FillRect(src, &pt, SDL_MapRGBA(src->format, pal[index[pix]].r,
                                               pal[index[pix]].g,
                                               pal[index[pix]].b, alpha[pix]));
          }
        }
      } else {
...
SimpleTexture::SimpleTexture(const SimpleTexture &in) { CopyFrom(in); }
...
void SimpleTexture::BuildBlankTexture() {
  ...
  if (type == SIMPLETEXTURE_COLOR) {
    cur = SDL_CreateRGBSurface(SDL_SWSURFACE, xsize, ysize, 32,
                               ST_SDL_RGBA_COLFIELDS);
...
void SimpleTexture::BuildTextTexture() {
  ...
  if (type == SIMPLETEXTURE_COLOR) {
    cur = SDL_CreateRGBSurface(SDL_SWSURFACE, xsize, ysize, 32,
                               ST_SDL_RGBA_COLFIELDS);
    cur = texturator->Generate(cur, Uint32(xsize * xfact),
                               Uint32(ysize * yfact), col);
  } else if (type == SIMPLETEXTURE_TRANSCOLOR) {
    cur = SDL_CreateRGBSurface(SDL_SWSURFACE, xsize, ysize, 32,
                               ST_SDL_RGBA_COLFIELDS);
    cur = invisible_texturator->Generate(cur, Uint32(xsize * xfact),
                                         Uint32(ysize * yfact), col);
  } else if (type == SIMPLETEXTURE_DEFINED) {
    cur = SDL_CreateRGBSurface(SDL_SWSURFACE, xsize, ysize, 32,
                               ST_SDL_RGBA_COLFIELDS);
  ...
  if (text->rendered_text == NULL) {  // Create persistent text surface
    ...
    text->rendered_text = SDL_CreateRGBSurface(
        SDL_SWSURFACE, text->text_xsize, text->text_ysize,
        32,                                               // FIXME: SDL Bug
    ...
    for (int ln = 0; ln < int(text->lines.size()); ++ln) {
      if (text->lines[ln].length() > 0) {
        ...
        if (text->alignment == ST_ALIGN_RIGHT) {
          int xs = 0, ys = 0;
          TTF_SizeText(Font(text->font_size), text->lines[ln].c_str(), &xs,
                       &ys);
...

  {
    ...
    if (type == SIMPLETEXTURE_TRANS || type == SIMPLETEXTURE_TRANSCOLOR) {
...
void SimpleTexture::Update() {
  if (type != SIMPLETEXTURE_NONE) {
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, cur->w, cur->h, GL_RGBA,
                      GL_UNSIGNED_BYTE, cur->pixels);
...
void SimpleTexture::SetText(const string txt) {
  ...
  text->text_ysize =
      TTF_FontLineSkip(Font(text->font_size)) * text->lines.size();
...
void SimpleTexture::SetColor(const int col_index, const float r, const float g,
                             const float b, const float tr, const float tg,
                             const float tb) {
  SimpleTexture::InitStatic();
  if ((int)(color.size()) <= (col_index * 2 + 1)) {
    fprintf(stderr, "ERROR: Color index '%d' out of range!\n", col_index);
...
void SimpleTexture::SetDefaultTextColor(const float tr, const float tg,
                                        const float tb) {
...
TTF_Font *SimpleTexture::CurrentFont() {
  if (text)
    return Font(text->font_size);
  else
    return Font(default_pxsize);
}

TTF_Font *SimpleTexture::Font(int pxsz) {
  if (pxsz < 1) pxsz = default_pxsize;
  if (cur_font.count(pxsz))
...
float SimpleTexture::ScaleX(float base) { return base * xfact; }

float SimpleTexture::ScaleY(float base) { return base * yfact; }
...
int SimpleTexture::ColorByName(const string &cname) {
  ...
  return NewColor((col >> 16) / 255.0, ((col >> 8) & 255) / 255.0,
                  (col & 255) / 255.0);
}

void SimpleTexture::EnableCursor() { show_cursor = true; }

void SimpleTexture::DisableCursor() { show_cursor = false; }
...
static long _zzip_seek(SDL_RWops *context, long offset, int whence) {
  return zzip_seek((ZZIP_FILE *)(context->hidden.unknown.data1), offset,
                   whence);
}

static size_t _zzip_read(SDL_RWops *context, void *ptr, size_t size,
                         size_t maxnum) {
  return (zzip_read((ZZIP_FILE *)(context->hidden.unknown.data1), (char *)ptr,
                    size * maxnum) /
          size);
}

static size_t _zzip_write(SDL_RWops *context, const void *ptr, size_t size,
                          size_t num) {
  return 0; /* ignored */
}
