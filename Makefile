...
CXX=	$(ARCH)-g++
FLAGS=	--std=c++11 -g -Wall -Werror `pkg-config sdl2 --cflags` `pkg-config zziplib --cflags`
...
debug:
	$(MAKE) FLAGS='-g -Wall -Werror -DSDL_DEBUG=SDL_INIT_NOPARACHUTE `pkg-config sdl2 --cflags` `pkg-config zziplib --cflags`' LIBS='`pkg-config sdl --libs` `pkg-config zziplib --libs` -lGL -lGLU'

#DEGUGGING OPTIONS (WITH EFENCE)
efence:
	$(MAKE) FLAGS='-g -Wall -Werror -DSDL_DEBUG=SDL_INIT_NOPARACHUTE `pkg-config sdl2 --cflags` `pkg-config zziplib --cflags`' LIBS='`pkg-config sdl --libs` `pkg-config zziplib --libs` -lefence -lGL -lGLU'

#PROFILING OPTIONS
prof:
	$(MAKE) FLAGS='-pg -g -Wall -Werror -DSDL_DEBUG=SDL_INIT_NOPARACHUTE `pkg-config sdl2 --cflags` `pkg-config zziplib --cflags`' LIBS='`pkg-config sdl --libs` `pkg-config zziplib --libs` -lGL -lGLU'

#PRODUCTION OPTIONS (WORKAROUND FOR MacOS-X)
macos:
	$(MAKE) FLAGS='-s -O2 -Wall -Werror `pkg-config sdl2 --cflags` `pkg-config zziplib --cflags`' LIBS='`pkg-config sdl --libs` `pkg-config zziplib --libs` -framework OpenGL'

#PRODUCTION OPTIONS (WORKAROUND FOR CYGWIN)
cygwin:
	$(MAKE) FLAGS='-s -O2 -Wall -Werror `pkg-config sdl2 --cflags` `pkg-config zziplib --cflags`' LIBS='`pkg-config sdl --libs` `pkg-config zziplib --libs` -L/usr/X11R6/bin -lopengl32 -lglu32'

#PRODUCTION OPTIONS (CROSS-COMPILED FOR WINDOWS)
WARCH=	i586-mingw32msvc
WCXX=	i586-mingw32msvc-g++
WFLAGS=	-s -O2 -Wall -Werror `/opt/i586-mingw32msvc/bin/pkg-config sdl2 --cflags` `/opt/i586-mingw32msvc/bin/pkg-config zziplib --cflags` -I/opt/i586-mingw32msvc/include
