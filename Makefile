...
CXX=	$(ARCH)-g++
FLAGS=	-s -O2 -Wall `sdl-config --cflags` `pkg-config zziplib --cflags`
LIBS=	`sdl-config --libs` `pkg-config zziplib --libs` -lGL -lGLU

#DEGUGGING OPTIONS (NO EFENCE)
debug:
	make FLAGS='-g -Wall -DSDL_DEBUG=SDL_INIT_NOPARACHUTE `sdl-config --cflags` `pkg-config zziplib --cflags`' LIBS='`sdl-config --libs` `pkg-config zziplib --libs` -lGL -lGLU'

#DEGUGGING OPTIONS (WITH EFENCE)
efence:
	make FLAGS='-g -Wall -DSDL_DEBUG=SDL_INIT_NOPARACHUTE `sdl-config --cflags` `pkg-config zziplib --cflags`' LIBS='`sdl-config --libs` `pkg-config zziplib --libs` -lefence -lGL -lGLU'

#PROFILING OPTIONS
prof:
	make FLAGS='-pg -g -Wall -DSDL_DEBUG=SDL_INIT_NOPARACHUTE `sdl-config --cflags` `pkg-config zziplib --cflags`' LIBS='`sdl-config --libs` `pkg-config zziplib --libs` -lGL -lGLU'

#PRODUCTION OPTIONS (WORKAROUND FOR MacOS-X)
macos:
	make FLAGS='-s -O2 -Wall `sdl-config --cflags` `pkg-config zziplib --cflags`' LIBS='`sdl-config --libs` `pkg-config zziplib --libs` -framework OpenGL'

#PRODUCTION OPTIONS (WORKAROUND FOR CYGWIN)
cygwin:
	make FLAGS='-s -O2 -Wall `sdl-config --cflags` `pkg-config zziplib --cflags`' LIBS='`sdl-config --libs` `pkg-config zziplib --libs` -L/usr/X11R6/bin -lopengl32 -lglu32'

#PRODUCTION OPTIONS (CROSS-COMPILED FOR WINDOWS)
WARCH=	i586-mingw32msvc
WCXX=	i586-mingw32msvc-g++
WFLAGS=	-s -O2 -Wall `/opt/i586-mingw32msvc/bin/sdl-config --cflags` `/opt/i586-mingw32msvc/bin/pkg-config zziplib --cflags` -I/opt/i586-mingw32msvc/include
WLIBS=	`/opt/i586-mingw32msvc/bin/sdl-config --libs` -lSDL -lpng -ljpeg -lpng `/opt/i586-mingw32msvc/bin/pkg-config zziplib --libs` -lopengl32 -lglu32
