...
void SimpleScene::SetModelIdleAnim(SS_Model mod, const string &an,
                                   int submodel) {
  SetModelAnimInt(mod, -1000, an, submodel);
}

void SimpleScene::SetModelMoveAnim(SS_Model mod, const string &an,
                                   int submodel) {
  SetModelAnimInt(mod, -1001, an, submodel);
}

void SimpleScene::SetModelTurnAnim(SS_Model mod, const string &an,
                                   int submodel) {
  SetModelAnimInt(mod, -1002, an, submodel);
}

void SimpleScene::SetModelAnim(SS_Model mod, int id, const string &an,
                               int submodel) {
  if (id <= -1000) {
...
void SimpleScene::SetModelAnimInt(SS_Model mod, int id, const string &an,
                                  int submodel) {
  if (models[mod].model) {
    ...
    models[mod].animmap.insert(
        pair<int, pair<int, int> >(id, pair<int, int>(anim, submodel)));
  }
}
...
void SimpleScene::ActObject(SS_Object obj, SS_Action act, Uint32 fin,
                            Uint32 dur) {
...
void SimpleScene::SkinObject(SS_Object obj, SS_Skin skin, Uint32 end,
                             Uint32 dur) {
  ActionTime act = {end, dur};
  if (skin != SS_UNDEFINED_SKIN)
...
void SimpleScene::ModelObject(SS_Object obj, SS_Model mod, Uint32 end,
                              Uint32 dur) {
  ActionTime act = {end, dur};
  if (mod != SS_UNDEFINED_MODEL)
...
void SimpleScene::SetPTypeColor0(SS_PType tp, float cr, float cg, float cb,
                                 float ca) {
...
void SimpleScene::SetPTypeColor1(SS_PType tp, float cr, float cg, float cb,
                                 float ca) {
...
void SimpleScene::SetPTypeColor(SS_PType tp, float cr, float cg, float cb,
                                float ca) {
  ptypes[(int)(tp)].cr0 = cr;
  ptypes[(int)(tp)].cg0 = cg;
  ptypes[(int)(tp)].cb0 = cb;
...
void SimpleScene::SetParticlePosition(SS_Particle part, float xp, float yp,
                                      float zp) {
...
void SimpleScene::GetObjectPos(SS_Object obj, Uint32 offset, float &xpos,
                               float &ypos) {
  float tmp;
...
bool SimpleScene::DrawObjects(Uint32 offset) {
  ...
  for (; obj != moves.end(); ++obj) {
    ...
    if (anims[0] < 0) anims[0] = models[model].model->LookUpAnimation("Stand");
    if (anims[0] < 0)
      anims[0] = models[model].model->LookUpAnimation("Stand 1");
    ...
    if (anims[1] < 0) anims[1] = models[model].model->LookUpAnimation("Stand");
    if (anims[1] < 0)
      anims[1] = models[model].model->LookUpAnimation("Stand 1");
    ...
    float ang = 0.0;
    {
      ...
      for (; turn != objects[obj->second.obj].turns.end(); ++turn) {
        if (offset < turn->second.finish) {
          ...
          oang = turn->first;
          tprog =
              (float)(offset + turn->second.duration - turn->second.finish) /
        ...
        if (tprog > 0.0) {  // Turning
          ...
          if (anims[0] < 0)
            anims[0] = models[model].model->LookUpAnimation("TURN");
          anim_sel = -1002;
        }
        break;
      }
    ...
    if (model != SS_UNDEFINED_MODEL) {
      ...
      if (models[model].animmap.count(anim_sel) > 0) {
        ...
        for (; anim != models[model].animmap.upper_bound(anim_sel); ++anim) {
          ...
          times[anim->second.second] =
              act->second.finish - act->second.duration;
        }
      }
