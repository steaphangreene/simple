...
SimpleGUI::SimpleGUI(int aspmeth, const float asp) {
  if (current_sg) {
    fprintf(stderr,
            "ERROR: Attempted to create multiple SimpleGUI instances.\n");
    exit(1);
  }
  current_sg = this;
...
bool SimpleGUI::ProcessEvent(SDL_Event *event) {
  ...
  if (event->type == SDL_WINDOWEVENT &&
    ...
    SimpleTexture::NeedToReacquireContext(event->window.data1,
                                          event->window.data2);
...
void SimpleGUI::SetFont(TTF_Font *font) { SimpleTexture::SetFont(font); }
...
const TTF_Font *SimpleGUI::Font(int pxsz) { return SimpleTexture::Font(pxsz); }
...
SimpleGUI *SimpleGUI::Current() { return current_sg; }
...
float SimpleGUI::Red(int c) { return SimpleTexture::Red(c); }

float SimpleGUI::Green(int c) { return SimpleTexture::Green(c); }

float SimpleGUI::Blue(int c) { return SimpleTexture::Blue(c); }

float SimpleGUI::TextRed(int c) { return SimpleTexture::TextRed(c); }

float SimpleGUI::TextGreen(int c) { return SimpleTexture::TextGreen(c); }

float SimpleGUI::TextBlue(int c) { return SimpleTexture::TextBlue(c); }

const SDL_Color *SimpleGUI::BGColor(int c) { return SimpleTexture::BGColor(c); }
...
void SimpleGUI::SetColor(int c, float r, float g, float b, float tr, float tg,
                         float tb) {
  SimpleTexture::SetColor(c, r, g, b, tr, tg, tb);
}

int SimpleGUI::NewColor() { return SimpleTexture::NewColor(); }

int SimpleGUI::NewColor(float r, float g, float b, float tr, float tg,
                        float tb) {
  return SimpleTexture::NewColor(r, g, b, tr, tg, tb);
}

SG_Alignment *SimpleGUI::MasterWidget() { return mainWid; }

SG_Alignment *SimpleGUI::PopupWidget() { return popWid; }
