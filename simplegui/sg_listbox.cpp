...
SG_ListBox::SG_ListBox(const vector<SG_Widget*>& items, SimpleTexture desel,
                       SimpleTexture sel, SimpleTexture click,
                       SimpleTexture dis, unsigned int min, unsigned int max,
                       bool vert, float border)
    : SG_Compound((!vert ? (items.size() == 0 ? 1 : items.size()) : 1),
                  (vert ? (items.size() == 0 ? 1 : items.size()) : 1), 0, 0),
      texdesel(desel),
      texsel(sel),
      texclick(click),
      texdisable(dis) {
  ...
  if (min > max) {
    fprintf(stderr,
            "SG_ListBox Constructor: Invalid min %d because it is greater than "
            "max %d\n",
            min, max);
    exit(1);
  }
...
void SG_ListBox::AddItem(SG_Widget* w, int at) {
  ...
  a->Enable();
  SG_StickyButton* sb =
      new SG_StickyButton("", texdesel, texdisable, texclick, texsel);
  a->SetBackground(sb);
  a->AddWidget(w);
  ...
  if (at == -1 || at >= (signed)listsize) {
    ...
    if (vertical)
      AddWidget(a, 0, listsize - 1);
    else
      AddWidget(a, listsize - 1, 0);
  } else {
