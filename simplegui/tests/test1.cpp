...
int main(int argc, char **argv) {
  ...
  for (int tx = 0; tx < NUM_FRAMES; ++tx) {  // Create NUM_FRAMES actual
                                             // textures
    SDL_Surface *nsurf;
  ...
  button[0] =
      new SG_Button("Red", gui->NewColor(1.0, 0.0, 0.0),
                    gui->NewColor(1.0, 0.5, 0.5), gui->NewColor(1.0, 0.0, 0.0));
  button[1] =
      new SG_Button("Green", gui->NewColor(0.0, 1.0, 0.0),
                    gui->NewColor(0.5, 1.0, 0.5), gui->NewColor(0.0, 1.0, 0.0));
  button[2] =
      new SG_Button("Blue", gui->NewColor(0.0, 0.0, 1.0),
                    gui->NewColor(0.5, 0.5, 1.0), gui->NewColor(0.0, 0.0, 1.0));
  button[3] = new SG_StickyButton(
      "Yellow", gui->NewColor(1.0, 1.0, 0.0), gui->NewColor(0.5, 0.5, 0.0),
      gui->NewColor(1.0, 1.0, 0.0), gui->NewColor(0.5, 0.5, 0.0));
  button[3]->SetVisibleSize(SG_AUTOSIZE, SG_AUTOSIZE);
  button[4] = new SG_StickyButton(
      "Cyan", gui->NewColor(0.0, 1.0, 1.0), gui->NewColor(0.0, 0.5, 0.5),
      gui->NewColor(0.0, 1.0, 1.0), gui->NewColor(0.0, 0.5, 0.5));
  button[5] = new SG_StickyButton(
      "",  // Blank!
  ...
  text = new SG_TextArea("Text Area Is Here", gui->NewColor(0.6, 0.4, 0.3));
  ...
  prog = new SG_ProgressBar("Progress", gui->NewColor(0.3, 0.3, 0.3),
                            gui->NewColor(0.6, 0.0, 0.0));
      ...
      if (event.type == SDL_SG_EVENT) {
        if (event.user.code == SG_EVENT_BUTTONCLICK) {
          ...
          if (event.user.data1 == (void *)button[2]) {
            ...
            anum %= 3;
            if (anum == 0)
              edit->SetAlignment(SG_ALIGN_LEFT);
            else if (anum == 1)
              edit->SetAlignment(SG_ALIGN_CENTER);
            else if (anum == 2)
              edit->SetAlignment(SG_ALIGN_RIGHT);
          }
          ...
          printf(
              "Received SG_EVENT_BOX from %s, for button #%d"
              " - for area (%f,%f) - (%f,%f).\n",
              name[(SG_PassThrough *)(event.user.data1)].c_str(),
              event.user.code - SG_EVENT_BOX, ((float *)(event.user.data2))[2],
