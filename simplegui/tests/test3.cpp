    ...
    do {  // while(gui->PollEvent(&event))
      if (event.type == SDL_SG_EVENT) {
        if (event.user.code == SG_EVENT_BUTTONCLICK) {
          ...
          if (s == "vaddat") {
            listb->AddItem(new SG_TransLabel(vaddval->Text(), col),
                           atoi(vaddind->Text().c_str()));
          }
          if (s == "haddat") {
            hlistb->AddItem(new SG_TransLabel(haddval->Text(), col),
                            atoi(haddind->Text().c_str()));
            l = hlistb;
          }
          if (s == "vrem") {
            if (listb->RemoveItem(atoi(vremind->Text().c_str())))
              fprintf(stderr, "Removal Successful\n");
            else
              fprintf(stderr, "Removal Unsuccessful\n");
          }
          if (s == "hrem") {
            l = hlistb;
            if (hlistb->RemoveItem(atoi(hremind->Text().c_str())))
              fprintf(stderr, "Removal Successful\n");
            else
              fprintf(stderr, "Removal Unsuccessful\n");
          }
          ...
          printf("Received SG_EVENT_EDITCOMPLETE - value = '%s'.\n",
                 ((SG_Text *)(event.user.data1))->Text().c_str());
          audio_play(click, 8, 8);
        } else if (event.user.code == SG_EVENT_SELECT_ITEM ||
