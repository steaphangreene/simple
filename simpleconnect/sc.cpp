...
SimpleConnect::SimpleConnect()
  ...
  SG_Widget *labelb = new SG_TextArea("Current Game", SG_COL_LOW);
      ...
      if (host->second.changed) {
        ...
        if (!name) {
          ...
          sprintf(buf, "%d.%d.%d.%d%c",
                  ...
                  ((unsigned char *)(&(host->second.address.host)))[3], 0);
          name = buf;
        }
        AddWidget(new SG_TextArea(name), 0, HEADER_SIZE + host->second.line, 2,
                  1);
        AddWidget(new SG_TextArea(host->second.mapname), 2,
                  HEADER_SIZE + host->second.line, 4, 1);
...
void SimpleConnect::InitSlots() {
  ...
  for (unsigned int slot = 0; slot < conn.slots.size(); ++slot) {
    ...
    pnamec->SetLimits(0.0, -1.0 * slot, 0.0,
                      1.0 * (conn.slots.size() - slot - 1));
    pnamec->SetDisplayLimits(
        0.0, -2.0 * slot / (1.0 - VERT_MARGIN), 0.0,
        2.0 * (conn.slots.size() - slot - 1) / (1.0 - VERT_MARGIN));
    ...
    if (event->user.code == SG_EVENT_BUTTONPRESS) {
      if (event->user.data1 == (void *)(SG_Widget *)(startb)) {
        ...
        conn.slots[teammap[(SG_Widget *)event->user.data1]].team =
            NextTeam(conn.slots[teammap[(SG_Widget *)event->user.data1]].team);
        slots_dirty = true;
        if (mode == SC_MODE_HOST) {
          slots_send = true;
          SDL_mutexV(net_mutex);
  ...
  while (!exiting) {
    ...
    for (; slots_handled < conn.net->NumConnections(); ++slots_handled) {
      ...
      for (; slot != conn.slots.end(); ++slot) {
        if ((slot->ptype == SC_PLAYER_NONE || slot->ptype == SC_PLAYER_AI) &&
            (slot->type == SC_SLOT_PLAYER || slot->type == SC_SLOT_HUMANONLY ||
    ...
    if (slots_send) {
      ...
      for (int sock = 0; sock < conn.net->NumConnections(); ++sock) {
        if (conn.net->IsConnected(sock) == SN_CONN_OK) {
          for (unsigned int slot = 0; slot < conn.slots.size(); ++slot)
            if (conn.slots[slot].sock == sock)
              data[slot * 20 + 1] = SC_PLAYER_LOCAL;
    ...
    for (int sock = 0; sock < conn.net->NumConnections(); ++sock) {
      if (conn.net->RecvBufferSize(sock) > 0) {
        ...
        if (type == SC_ACT_CHANGECOLOR) {
          ...
          toremove.insert(sock);
          fprintf(stderr,
                  "ERROR: Got unknown data from socket, and closed it\n");
        }
      }
    }

    for (unsigned int slot = 0; slot < conn.slots.size(); ++slot) {
      if (conn.slots[slot].sock >= 0 &&
          conn.net->IsConnected(conn.slots[slot].sock) != SN_CONN_OK) {
        toremove.insert(conn.slots[slot].sock);
      }
    }

    for (set<int>::iterator sock = toremove.begin(); sock != toremove.end();
         ++sock) {
      SDL_mutexP(net_mutex);
      vector<SlotData>::iterator slot = conn.slots.begin();
      for (; slot != conn.slots.end(); ++slot) {
...
void SimpleConnect::StartNet() {
  ...
  if (mode == SC_MODE_SEARCH)
    net_thread =
        SDL_CreateThread(search_thread_handler, "search", (void *)(this));
  else if (mode == SC_MODE_HOST)
    net_thread = SDL_CreateThread(host_thread_handler, "host", (void *)(this));
  else if (mode == SC_MODE_SLAVE)
    net_thread =
        SDL_CreateThread(slave_thread_handler, "slave", (void *)(this));
}
...
int SimpleConnect::NextFreeColor(int oldcolor) {
  ...
  for (col = 1; col < colors.size(); ++col) {
    if (((col + oldcolor) % colors.size() != 0) &&
        (!taken[(col + oldcolor) % colors.size()]))
      break;
  }
...
int SimpleConnect::NextTeam(int oldteam) {
  ...
  bool not_seen = true;
  for (unsigned int slot = 0; slot < conn.slots.size(); ++slot) {
    if (not_seen && conn.slots[slot].team == oldteam)
      not_seen = false;
    else if (maxteam < conn.slots[slot].team)
      maxteam = conn.slots[slot].team;
  }
...
const string &SimpleConnect::PlayerName() { return nameb->Text(); }
