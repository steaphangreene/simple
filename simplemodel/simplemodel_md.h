...
class SimpleModel_MD : public SimpleModel {
 ...
 protected:
  ...
  enum InterpolationTypes {
    InterpolationNone = 0,
    InterpolationTypeLinear = 1,
    InterpolationTypeHermite = 2,
    InterpolationTypeBezier = 3
  };
  ...
  struct MDXSequence {
    ...
    Uint32 end;
    float speed;
    bool loop;  // In the file it's a long with 0 meaning loop and 1 meaning no
                // loop
    float rarity;
    Uint32 unknown;
    float bound_radius;
  ...
  struct MDXBone {
    void CalcBoneTransform(Matrix4x4 &, const MDXVertex &, const Matrix4x4 &,
                           const AnimationInfo &) const;
    bool CalcBoneTranslation(MDXVertex &, const AnimationInfo &) const;
    bool CalcBoneRotation(Quaternion &, const AnimationInfo &) const;
    void LERP(MDXVertex &, const MDXVertex &, const MDXVertex &,
              const float) const;
  ...
  struct MDXGeosetAnim {
    ...
    Uint32 color_animation;  // none = 0, drop shadow = 1, color = 2, both = 3,
                             // default is 1
    float color[3];          // R, G, B
    Uint32 geoset_id;
  ...
  struct MDXGeoset {
    ...
    void AccumulateBoneTransforms(TransformInfo &, const Uint32, const Uint32,
                                  const Uint32) const;
    ...
    Uint32
        selectable;  // Doesn't seem relevant to us, none = 0, 4 = unselectable
    float bound_radius;
    float min_extents[3];
    float max_extents[3];
  ...
  SimpleModel_MD();
  void CalcTransforms(TransformInfo &, const Matrix4x4 &, const Sint32,
                      const AnimationInfo &) const;
