...
bool SimpleModel_OBJ::Load(const string &filenm, const string &texnm) {
  ...
  while ((ptr - buf) < size) {
    if (!strncmp(ptr, "f ", 2)) {
      ...
      int n[4] = {0, 0, 0, 0};
      int res;
      res = sscanf(ptr + 2, "%d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d", p + 0, t + 0,
                   n + 0, p + 1, t + 1, n + 1, p + 2, t + 2, n + 2, p + 3,
                   t + 3, n + 3);
      if (res < 9) {
        res = sscanf(ptr + 2, "%d//%d %d//%d %d//%d %d//%d", p + 0, n + 0,
                     p + 1, n + 1, p + 2, n + 2, p + 3, n + 3);
      }
      if (res < 6) {
        res = sscanf(ptr + 2, "%d/%d/ %d/%d/ %d/%d/ %d/%d/", p + 0, t + 0,
                     p + 1, t + 1, p + 2, t + 2, p + 3, t + 3);
      }
      if (res < 6) {
        res =
            sscanf(ptr + 2, "%d// %d// %d// %d//", p + 0, p + 1, p + 2, p + 3);
      }
      if (res < 3) {
  ...
  if (SimpleModel::glGenBuffersARB == NULL) {
    ...
    SimpleModel::glBufferDataARB(GL_ARRAY_BUFFER,
                                 sizeof(GLfloat) * buffer.size(), &(buffer[0]),
                                 GL_STATIC_DRAW_ARB);
...
void SimpleModel_OBJ::ReloadValidModels() {
  set<SimpleModel_OBJ *> temp_set = valid_models;
  valid_models.clear();
  set<SimpleModel_OBJ *>::iterator mod = temp_set.begin();
  for (; mod != temp_set.end(); ++mod) {
