...
SimpleModel_MDX::SimpleModel_MDX(const string &filenm, const string &modelnm,
...
bool SimpleModel_MDX::Load(const string &filenm, const string &modelnm,
                           const vector<string> &skins) {
  ...
  while ((token = GetNextToken(model)) != "EOB") {
    ...
    if (token == "VERS") {
      if (HandleVersion(model) == false) return false;
    } else if (token == "MODL") {
      if (HandleModel(model) == false) return false;
    } else if (token == "SEQS") {
      if (HandleSequences(model) == false) return false;
    } else if (token == "GLBS") {
      if (HandleGlobalSequences(model) == false) return false;
    } else if (token == "TEXS") {
      if (HandleTextures(filenm, model) == false) return false;
    } else if (token == "GEOS") {
      if (HandleGeosets(model) == false) return false;
    } else if (token == "MTLS") {
      if (HandleMaterials(model) == false) return false;
    } else if (token == "GEOA") {
      if (HandleGeosetAnims(model) == false) return false;
    } else if (token == "BONE") {
      if (HandleBones(model) == false) return false;
    } else if (token == "HELP") {
      if (HandleHelpers(model) == false) return false;
    } else if (token == "PIVT") {
      if (HandlePivots(model) == false) return false;
    } else if (token == "SKIP")
      SkipChunk(model);
    else {
...
bool SimpleModel_MDX::HandleSequences(SDL_RWops *model) {
  ...
  for (vector<MDXSequence>::iterator it = sequences.begin();
       it != sequences.end(); ++it) {
...
bool SimpleModel_MDX::HandleGlobalSequences(SDL_RWops *model) {
  ...
  for (vector<Uint32>::iterator it = global_sequences.durations.begin();
       it != global_sequences.durations.end(); ++it)
    freadLE(*it, model);
...
bool SimpleModel_MDX::HandleTextures(const string &filenm, SDL_RWops *model) {
  ...
  for (vector<MDXTexture>::iterator it = textures.begin(); it != textures.end();
       ++it) {
    ...
    if (it->replacable_id == 0) {
      ...
      texture.push_back(
          new SimpleTexture(SimpleTexture::NewColor(0.0, 0.0, 0.0)));
    } else {
...
bool SimpleModel_MDX::HandleGeosets(SDL_RWops *model) {
  ...
  while (bytes_read <= chunk_size) {
    ...
    SDL_RWseek(model, 4, SEEK_CUR);
    if (HandleVertices(model, new_geoset.vertices) == false) return false;

    if (HandleNormals(model, new_geoset.normals) == false) return false;

    if (HandlePrimTypes(model, new_geoset.prim_types) == false) return false;

    if (HandlePrimCounts(model, new_geoset.prim_counts) == false) return false;

    if (HandleIndices(model, new_geoset.indices) == false) return false;
    ...
    if (HandleMatrices(model, new_geoset.matrices) == false) return false;
    ...
    if (HandleFrameInfo(model, new_geoset.animations) == false) return false;
  ...
  while (bytes_read < chunk_size) {
    ...
    if (!strncmp((char *)tag_name, "KGAO", 4)) {
      ...
      for (Uint32 key = 0; key < alpha.nunks; ++key) {
        ...
        //	alpha.key_frames[key].frame, alpha.key_frames[key].state,
        // alpha.line_type);
      }
...
bool SimpleModel_MDX::HandleVertices(SDL_RWops *model,
  ...
  for (vector<MDXVertex>::iterator it = vertices.begin(); it != vertices.end();
       ++it) {
    freadLE(it->x, model);
    freadLE(it->y, model);
    freadLE(it->z, model);
...
bool SimpleModel_MDX::HandleNormals(SDL_RWops *model,
  ...
  for (vector<MDXNormal>::iterator it = normals.begin(); it != normals.end();
       ++it) {
...
bool SimpleModel_MDX::HandlePrimTypes(SDL_RWops *model,
  ...
  for (vector<Uint32>::iterator it = prim_types.begin(); it != prim_types.end();
       ++it)
    freadLE(*it, model);
...
bool SimpleModel_MDX::HandlePrimCounts(SDL_RWops *model,
  ...
  for (vector<Uint32>::iterator it = prim_counts.begin();
       it != prim_counts.end(); ++it)
    freadLE(*it, model);
...
bool SimpleModel_MDX::HandleVertexGroups(SDL_RWops *model,
  ...
  for (vector<Uint8>::iterator it = vertex_groups.begin();
       it != vertex_groups.end(); ++it)
    freadLE(*it, model);

  return true;
}

bool SimpleModel_MDX::HandleGroupCounts(SDL_RWops *model,
  ...
  for (vector<Uint32>::iterator it = group_counts.begin();
       it != group_counts.end(); ++it)
    freadLE(*it, model);

  return true;
}

bool SimpleModel_MDX::HandleMatrices(SDL_RWops *model,
  ...
  for (vector<Uint32>::iterator it = matrices.begin(); it != matrices.end();
       ++it)
    freadLE(*it, model);
...
bool SimpleModel_MDX::HandleFrameInfo(SDL_RWops *model,
                                      vector<MDXFrameInfo> &mdx_animations) {
  Uint32 num_anims = 0;

  freadLE(num_anims, model);
  mdx_animations.resize(num_anims);
  for (vector<MDXFrameInfo>::iterator it = mdx_animations.begin();
       it != mdx_animations.end(); ++it) {
    freadLE(it->bound_radius, model);
...
bool SimpleModel_MDX::HandleUVAS(SDL_RWops *model,
                                 vector<vector<MDXTextureCoord> > &uvas) {
  Uint32 num_tverts = 0;
  Uint8 chunk_name[4];
  ...
  for (vector<vector<MDXTextureCoord> >::iterator it = uvas.begin();
       it != uvas.end(); ++it)
    HandleUVBS(model, *it);
...
bool SimpleModel_MDX::HandleUVBS(SDL_RWops *model,
                                 vector<MDXTextureCoord> &uvbs) {
  Uint32 num_verts = 0;
  Uint8 chunk_name[4];
  ...
  for (vector<MDXTextureCoord>::iterator it = uvbs.begin(); it != uvbs.end();
       ++it) {
...
bool SimpleModel_MDX::HandleBones(SDL_RWops *model) {
  ...
  while (bytes_read <= chunk_size) {
    ...
    if (HandleObject(model, new_bone.object) == false) return false;
...
bool SimpleModel_MDX::HandleObject(SDL_RWops *model, MDXObject &object) {
  ...
  if (NextTokenIs(model, "KGTR")) {
    if (HandleKGTR(model, object.translation_info) == false) return false;
  }

  if (NextTokenIs(model, "KGRT")) {
    if (HandleKGRT(model, object.rotation_info) == false) return false;
  }

  if (NextTokenIs(model, "KGSC")) {
    if (HandleKGSC(model, object.scaling_info) == false) return false;
  }

  if (NextTokenIs(model, "KATV")) {
    if (HandleKATV(model, object.visibility_info) == false) return false;
  }

  return true;
}

bool SimpleModel_MDX::HandleKGTR(SDL_RWops *model,
                                 MDXTranslationInfo_KGTR &trans) {
  Uint32 nunks = 0;
...
bool SimpleModel_MDX::HandleKGRT(SDL_RWops *model, MDXRotationInfo_KGRT &rot) {
  ...
  for (Uint32 i = 0; i < nunks; ++i) {
    ...
    freadLE(rot.key_frames.at(i).quat.x, model);
    freadLE(rot.key_frames.at(i).quat.y, model);
    freadLE(rot.key_frames.at(i).quat.x, model);
    freadLE(rot.key_frames.at(i).quat.w, model);
    rot.key_frames.at(i).quat.w = -(rot.key_frames.at(i).quat.w);
...
bool SimpleModel_MDX::HandleKATV(SDL_RWops *model,
                                 MDXVisibilityInfo_KATV &vis) {
  Uint32 nunks = 0;
