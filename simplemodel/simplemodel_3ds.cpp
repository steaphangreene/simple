...
bool SimpleModel_3DS::Load(const string &filenm, const string &texnm) {
  ...
  Sint32 filelen = 0;
  Uint16 chunk_id = 0;  // Chunk's ID (3DS files are structured in chunks. We
    ...
    switch (chunk_id) {
      ...
      case 0x4100:
        if (first_mesh == true)
          ...
          current_mesh.has_texture = false;
          for (index = 0; index < 20; ++index) current_mesh.name[index] = '\0';
        }
        break;
      ...
      case 0x4120:
        ...
        for (index = 0; index < chunk_qty; ++index) {
          ...
          freadLE(face_flags,
                  model);  // Don't really need these, just reading them in
        }
        break;
  ...
  for (mesh_index = 0; mesh_index < mesh.size(); ++mesh_index) {
    for (index = 0; index < mesh[mesh_index].polygon.size(); ++index) {
      ...
      if (mesh[mesh_index].has_texture == true)
        glTexCoord2f(
            mesh[mesh_index].tex_coord[mesh[mesh_index].polygon[index].v1].u,
            mesh[mesh_index].tex_coord[mesh[mesh_index].polygon[index].v1].v);
      // Coordinates of the first vertex
      glVertex3f(mesh[mesh_index].vertex[mesh[mesh_index].polygon[index].v1].x,
                 mesh[mesh_index].vertex[mesh[mesh_index].polygon[index].v1].y,
                 mesh[mesh_index].vertex[mesh[mesh_index].polygon[index].v1].z);

      //----------------- SECOND VERTEX -----------------
      // Texture coordinates of the second vertex
      if (mesh[mesh_index].has_texture == true)
        glTexCoord2f(
            mesh[mesh_index].tex_coord[mesh[mesh_index].polygon[index].v2].u,
            mesh[mesh_index].tex_coord[mesh[mesh_index].polygon[index].v2].v);
      // Coordinates of the first vertex
      glVertex3f(mesh[mesh_index].vertex[mesh[mesh_index].polygon[index].v2].x,
                 mesh[mesh_index].vertex[mesh[mesh_index].polygon[index].v2].y,
                 mesh[mesh_index].vertex[mesh[mesh_index].polygon[index].v2].z);

      //----------------- THIRD VERTEX -----------------
      // Texture coordinates of the third vertex
      if (mesh[mesh_index].has_texture == true)
        glTexCoord2f(
            mesh[mesh_index].tex_coord[mesh[mesh_index].polygon[index].v3].u,
            mesh[mesh_index].tex_coord[mesh[mesh_index].polygon[index].v3].v);
      // Coordinates of the first vertex
      glVertex3f(mesh[mesh_index].vertex[mesh[mesh_index].polygon[index].v3].x,
                 mesh[mesh_index].vertex[mesh[mesh_index].polygon[index].v3].y,
                 mesh[mesh_index].vertex[mesh[mesh_index].polygon[index].v3].z);
    }
  }
  glEnd();
  glEndList();
