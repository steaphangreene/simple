...
// Create a Server
SimpleNetwork::SimpleNetwork(Uint16 prt) {
  if (current) {
...
SimpleNetwork::~SimpleNetwork() {
  if (recv_thread) {
    SDL_Thread *tmp = recv_thread;
    recv_thread = NULL;
    SDL_WaitThread(tmp, NULL);
  }
}
...
void SimpleNetwork::Send(int slot) {
  if (data.count(slot) > 0 && data[slot].conn_status == SN_CONN_OK) {
    ...
    int count;
    for (count = 0, i = send_buffer->begin(); i != send_buffer->end();
         ++i, ++count)
      tmp_buffer[count] = *i;
...
int SimpleNetwork::NumConnections() { return data.size(); }

// Disconnectes from the server by sending it the QUIT operator which will force
...
void SimpleNetwork::SetPort(Uint16 port) { this->port = port; }
...
int SimpleNetwork::Connect(IPaddress &ip, const string &name,
  ...
  SDLNet_TCP_Send(data[curr_slot].tcp, (void *)bytes,
                  password.length() + name.length() + 2);
...
void SimpleNetwork::UnlockConnections() { connections_locked = false; }

void SimpleNetwork::LockConnections() { connections_locked = true; }

void SimpleNetwork::StopAccepting() {
  SDL_mutexP(accept_mutex);
  if (accept_thread) {
    accept_amount = 0;
    SDL_Thread *tmp = accept_thread;
    accept_thread = NULL;
    SDL_WaitThread(tmp, NULL);
    SDLNet_TCP_Close(sd);
  }
...
// if not already accepting, this starts a thread which accepts 'amount number
// of
// new sockets.
  ...
  while (amt && accept_thread) {
    if ((csd = SDLNet_TCP_Accept(sd))) {
      ...
      if ((cnx_amt = SDLNet_TCP_AddSocket(cnx_set, csd)) == -1) {
        ...
        // for debugging code
        // fprintf(stderr, "Received Bytes (ACCEPT): ");
...
// runs the client thread which recieves information from the server if they
// send something
...
int SimpleNetwork::RunRecv() {
  ...
  while (recv_thread) {
    ...
    for (; i != data.end(); ++i) {
      ...
      if (d->conn_status != SN_CONN_OK) continue;

      // if its not ready we want to make sure its non-idle, assuming we are the
      // server.
      ...
      if (strcmp(cstr, "QUIT") == 0) {
        ...
        // if a NOOP is sent, an OKOP verification is sent back, this is to
        // assert
        // that their is still a live connection between the server and the
        // client.
      ...
      } else if (strcmp(cstr, "OKOP") !=
                 0) {  // do nothing if OKOP otherwise this.
