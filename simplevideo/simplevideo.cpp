...
void SimpleVideo::EnableLighting() { glEnable(GL_LIGHTING); }

void SimpleVideo::DisableLighting() { glDisable(GL_LIGHTING); }

void SimpleVideo::SetOrtho() { flags = SV_ORTHO; }

void SimpleVideo::SetPerspective(float vert_fov) {
  if (vert_fov > 360.0)
    vert_fov = 360.0;
  else if (vert_fov < 0.0)
    vert_fov = 0.0;
...
bool SimpleVideo::StartScene() {
  ...
  gluLookAt(xvp + x + xoff, yvp + y + yoff, zvp + z, x + xoff, y + yoff, z, upx,
            upy, upz);
...
void SimpleVideo::SetScene(SimpleScene *s) { scene = s; }

void SimpleVideo::SetGUI(SimpleGUI *g) { gui = g; }
...
void SimpleVideo::ResetSubscreen() { SetSubscreen(-1.0, -1.0, 1.0, 1.0); }
...
const vector<SimpleVideo_Mode> SimpleVideo::GetFullScreenModes() const {
  ...
  //  SDL_Rect **sdl_modes = SDL_ListModes(NULL,
  // window_flags|SDL_WINDOW_FULLSCREEN);
...
void SimpleVideo::GetViewLimits(float &x0, float &y0, float &z0, float &x1,
                                float &y1, float &z1) {
  z0 = minz;
  z1 = maxz;
